...
class Question {
    ...
    option2: string
    option3: string
    constructor(prompt: string, answer: string, option0: string, option1: string, option2: string, option3: string) {
        this.prompt = prompt;
        this.answer = answer;
...
function renderOptions (questionVariable: Question) {
    ...
    for (let i = 0; i < prompts.length; i++) {
        prompts[i].addEventListener('click', function(e) {
            ...
            if (confirmation) {
                if ((answers[i] as HTMLInputElement).value == solutionVar) {
                    alert("Good job!");
                    score++;
                    currentIndex++;
                ...
                } else if ((answers[i] as HTMLInputElement).value !== solutionVar) {
...
function insertAfter(newNode, existingNode: HTMLElement) {
